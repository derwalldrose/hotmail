...
def load_runtime_settings():
    config = get_config()
    return {
        "email_suffix": config.get("email_suffix", "@outlook.com"),
        "proxy_source": config.get("proxy_source", "api"),
        ...
        "enable_oauth2": config.get("oauth2", {}).get("enable_oauth2", False),
        "client_id": config.get("oauth2", {}).get("client_id", ""),
        "redirect_url": config.get("oauth2", {}).get("redirect_url", "http://localhost:8000"),
...
if __name__ == "__main__":
    ...
    status_bar.show_logs = runtime_settings["show_logs"]

    if runtime_settings["show_logs"]:
    ...
    try:
        run_concurrent_flows(
            controller,
            runtime_settings["concurrent_flows"],
            runtime_settings["max_tasks"],
        )
    finally:
