...
def _build_proxy_capacity_map(proxies):
    capacity_map = {}
    for proxy_config in proxies:
        key = _get_proxy_key(proxy_config)
        capacity_map[key] = capacity_map.get(key, 0) + 1
    return capacity_map


def reserve_proxy(proxy_config, capacity=1):
    if not proxy_config:
        return False

    key = _get_proxy_key(proxy_config)
    with _proxy_assignment_lock:
        active_count = _active_proxy_counts.get(key, 0)
        if active_count >= capacity:
            return False
        _active_proxy_counts[key] = active_count + 1
        return True
...
def release_proxy(proxy_config):
    ...
    with _proxy_assignment_lock:
        active_count = _active_proxy_counts.get(key, 0)
        if active_count <= 1:
            _active_proxy_counts.pop(key, None)
            return
        _active_proxy_counts[key] = active_count - 1


def _is_proxy_active(proxy_config, capacity=1):
    if not proxy_config:
        return False

    key = _get_proxy_key(proxy_config)
    with _proxy_assignment_lock:
        return _active_proxy_counts.get(key, 0) >= capacity
...
def fetch_api_proxy_assignment():
    ...
    for _ in range(4):
        ...
        capacity_map = _build_proxy_capacity_map(candidate_pool)
        random.shuffle(candidate_pool)

        for proxy in candidate_pool:
            capacity = capacity_map.get(_get_proxy_key(proxy), 1)
            if not _is_proxy_active(proxy, capacity=capacity):
                return proxy
...
def get_working_proxy(file_path=None, preferred_proxy=None, reserve=False):
    ...
    capacity_map = _build_proxy_capacity_map(proxies)
    ...
    for proxy_config in proxies:
        key = _get_proxy_key(proxy_config)
        capacity = capacity_map.get(key, 1)
        with _proxy_health_lock:
            if key in _bad_proxy_keys:
                continue
        if _is_proxy_active(proxy_config, capacity=capacity):
            continue
        ...
        if failed_url is None:
            if reserve and not reserve_proxy(proxy_config, capacity=capacity):
                continue
            return proxy_config
